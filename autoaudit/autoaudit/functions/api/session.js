...
export async function onRequestGet(context) {
  ...
  try {
    ...
    const results = await env.DB.prepare(
      "SELECT ctrl_id, ctrl_name, verdict, finding, recommendation, COALESCE(evidence_type,'none') as evidence_type FROM gap_results WHERE session_id=? ORDER BY id ASC"
    ).bind(id).all();
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (action === "result") {
      const { session_id, ctrl_id, ctrl_name, verdict, finding, recommendation, evidence_type } = body;
      if (!session_id || !ctrl_id) return err("Missing session_id or ctrl_id");

      await env.DB.prepare(`
        INSERT INTO gap_results (session_id, ctrl_id, ctrl_name, verdict, finding, recommendation, evidence_type)
        VALUES (?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT DO NOTHING
      `).bind(session_id, ctrl_id, ctrl_name, verdict, finding || "", recommendation || "", evidence_type || "none").run();

      return ok({});
    }

    // ── UPDATE an existing control result (Re-audit) ─────────────────────────
    if (action === "update-result") {
      const { session_id, ctrl_id, verdict, finding, recommendation, evidence_type } = body;
      if (!session_id || !ctrl_id) return err("Missing session_id or ctrl_id");

      await env.DB.prepare(`
        UPDATE gap_results
        SET verdict=?, finding=?, recommendation=?, evidence_type=?, assessed_at=CURRENT_TIMESTAMP
        WHERE session_id=? AND ctrl_id=?
      `).bind(verdict, finding || "", recommendation || "", evidence_type || "none", session_id, ctrl_id).run();
